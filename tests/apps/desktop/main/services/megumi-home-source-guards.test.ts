...
describe('Megumi Home source guards', () => {
  ...
  it('creates provider encrypted secret files under secrets/providers', () => {
    const source = readProjectFile('apps/desktop/src/main/services/secret-store.service.ts');
  ...
  it('does not register legacy AI, agent, or settings IPC handlers', () => {
    ...
    expect(source).not.toContain('./handlers/agent.handler');
