...
describe('createBranchDraftViewInput', () => {
  it('uses the assistant reply text when the branched reply has visible answer text', () => {
    ...
    expect(createBranchDraftViewInput(assistant, [
      userMessage({ messageId: 'message-user-1', executionId: 'run-1', text: '你是谁？' }),
      assistant,
    ])).toEqual({
      messageId: 'message-assistant-1',
      sourceKind: 'reply',
  ...
  it('falls back to the same-run user input when the reply has no visible answer text', () => {
    ...
    expect(createBranchDraftViewInput(assistant, [
      userMessage({ messageId: 'message-user-other', executionId: 'run-other', text: '不要使用这一条' }),
      userMessage({ messageId: 'message-user-1', executionId: 'run-1', text: '解释这个报错' }),
      assistant,
    ])).toEqual({
      messageId: 'message-assistant-1',
      sourceKind: 'input',
