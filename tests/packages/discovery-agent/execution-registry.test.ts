...
import { Agent, type AgentOptions } from '@megumi/agent';
...
function stubFailure(): import('../../../packages/discovery-agent/src/execution/execution-registry').ExecutionFailure {
...
async function attachExecutingToolsAgent(
  ...
  readonly execution: Promise<import('@megumi/agent').AgentExecutionResult>;
}> {
  const hangingTool = {
    name: 'hang',
    description: 'Hang.',
