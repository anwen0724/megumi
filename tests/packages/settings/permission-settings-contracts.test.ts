...
describe('permission settings contracts', () => {
  it('stores sparse modes and structured rules without accepting legacy patterns', () => {
    ...
    expect(PermissionSettingsSchema.parse({ mode: 'ask', allow: [], ask: [], deny: [] })).toBeTruthy();
    expect(PermissionSettingsSchema.safeParse({ mode: 'custom', allow: [], ask: [], deny: [] }).success).toBe(false);
  });
