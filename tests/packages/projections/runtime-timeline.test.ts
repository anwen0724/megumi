...
describe('runtime timeline projection', () => {
  it('suppresses an already applied Runtime Event by event identity', () => {
    const textDelta = event('model_call.text_delta', {
      modelCallId: 'model-call:1',
      delta: 'only once',
    }, 1);
    const once = reduceRuntimeTimeline({
      timeline: createRuntimeTimeline(),
      event: textDelta,
    });
    const duplicate = reduceRuntimeTimeline({ timeline: once, event: textDelta });

    expect(duplicate).toBe(once);
    expect(JSON.stringify(duplicate.messages)).toContain('only once');
    expect(JSON.stringify(duplicate.messages)).not.toContain('only onceonly once');
  });
