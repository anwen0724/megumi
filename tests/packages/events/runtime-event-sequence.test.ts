/*
 * Protects sequence normalization and request-correlation helpers.
 */
...
} from '../../../packages/events/src/runtime-event-sequence';
...
describe('runtime event sequence helpers', () => {
  it('normalizes only missing sequence numbers', () => {
  ...
  it('finds the largest sequence and adds request metadata', () => {
    expect(lastRuntimeEventSequence([event(2), event(9), event(4)])).toBe(9);
    expect(withRuntimeRequestMetadata(event(1), {
      requestId: 'request:1',
      context: {
        requestId: 'request:1', traceId: 'trace-1', operationName: 'chat.send', source: 'renderer',
        createdAt: '2026-01-01T00:00:00.000Z',
      },
    })).toMatchObject({ requestId: 'request:1', context: { traceId: 'trace-1' } });
  });
});
