...
import type { ConversationTurn, CurrentConversationTurn } from '@megumi/coding-agent/context';
import { buildActiveContext } from '@megumi/coding-agent/context/service/internal/active-context-builder';
...
describe('buildActiveContext', () => {
  it('separates instructions from reference facts and emits non-visible source refs', () => {
    ...
    const result = buildActiveContext({
      ...
      skillCatalog: [{ skillId: 'skill-catalog', name: 'Catalog Skill', description: 'Available skill' }],
      activatedSkills: [{
    ...
    expect(result.activeContext.instructions).toEqual({
      ...
      activatedSkills: [{ skillId: 'skill-active', name: 'Active Skill', content: 'Activated instruction' }],
    });
