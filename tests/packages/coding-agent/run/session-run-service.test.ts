...
import { TimelineHistoryCommitProjectorService } from '@megumi/coding-agent/run/timeline-history-commit-projector';
import type {
...
describe('SessionRunService', () => {
  ...
  it('passes workspace baseline context to model step requests for session messages', async () => {
    ...
    const source = fs.readFileSync(path.join(process.cwd(), 'packages/coding-agent/run/session-run-service.ts'), 'utf8');
