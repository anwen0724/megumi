...
} from '@megumi/coding-agent/skills/service/internal/skill-path-policy';

describe('skill path policy', () => {
  const packagePath = path.resolve('C:/skills/checks');

  it('allows declared resource paths', () => {
    ...
    expect(validateSkillResourcePath({ packagePath, resourcePath: 'assets/logo.png' }).status).toBe('ok');
  });

  it('rejects path escapes, hidden files, and script reads as resources', () => {
