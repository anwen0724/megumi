...
const moduleRoot = path.join(repoRoot, 'packages/coding-agent/agent-run');
...
describe('agent-run module boundary', () => {
  it('exists and exposes only public module API from index.ts', async () => {
    ...
    const publicApi = await import('@megumi/coding-agent/agent-run');
