...
describe('migration failure boundary', () => {
  it('throws a typed error with sqlite and migrations paths', () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(CodingAgentDatabaseMigrationError);
      expect((error as CodingAgentDatabaseMigrationError).sqliteFile).toContain('megumi.sqlite3');
      expect((error as CodingAgentDatabaseMigrationError).migrationsFolder).toBe(migrationsFolder);
      expect((error as Error).message).toContain('Failed to apply Coding Agent database migrations');
    }
  });
});
