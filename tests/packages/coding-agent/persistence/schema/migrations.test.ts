...
describe('database migrations', () => {
  it('is idempotent', () => {
    ...
    expect(tableExists(database, 'sessions')).toBe(true);
    expect(tableExists(database, 'runs')).toBe(true);
  });
  ...
  it('does not create active agent-prefixed session run tables or indexes', () => {
    const source = readFileSync('packages/coding-agent/persistence/schema/migrations.ts', 'utf8');
