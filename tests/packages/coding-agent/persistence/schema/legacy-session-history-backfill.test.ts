...
import {
  CodingAgentDatabaseMigrationError,
  migrateCodingAgentDatabase,
} from '@megumi/coding-agent/persistence/schema';

const migrationsRoot = path.join(process.cwd(), 'packages/coding-agent/persistence/migrations');
...
describe('legacy Session history backfill', () => {
  it('rebuilds semantic Assistant and Tool Result messages in call order', () => {
    ...
    const migrated = migrateCodingAgentDatabase({
  ...
  it('rolls back staging and preserves legacy tables when a payload is malformed', () => {
    ...
    expect(() => migrateCodingAgentDatabase({
      sqliteDirectory: fixture.sqliteDirectory,
      migrationsFolder: migrationsRoot,
    })).toThrow(CodingAgentDatabaseMigrationError);
  ...
  it('fills a partially migrated run and remains idempotent on the next startup', () => {
    ...
    const first = migrateCodingAgentDatabase({ sqliteDirectory: fixture.sqliteDirectory, migrationsFolder: migrationsRoot });
    expect((first.database.prepare(`SELECT COUNT(*) AS count FROM session_messages`).get() as { count: number }).count)
      .toBe(5);
    first.database.close();

    const second = migrateCodingAgentDatabase({ sqliteDirectory: fixture.sqliteDirectory, migrationsFolder: migrationsRoot });
    try {
  ...
  it('retains an orphaned Tool Result as a semantic fact before the final Assistant message', () => {
    ...
    const migrated = migrateCodingAgentDatabase({ sqliteDirectory: fixture.sqliteDirectory, migrationsFolder: migrationsRoot });
    try {
...
function createLegacyFixture() {
  ...
  return {
    sqliteDirectory,
    database: migrateCodingAgentDatabase({ sqliteDirectory, migrationsFolder: legacyMigrations }).database,
  };
}
