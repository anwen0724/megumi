...
import { migrateCodingAgentDatabase } from '@megumi/coding-agent/persistence/schema';
...
describe('versioned database migrations', () => {
  it('applies only pending migrations and preserves existing data', () => {
    ...
    const first = migrateCodingAgentDatabase({ sqliteDirectory, migrationsFolder });
    ...
    const second = migrateCodingAgentDatabase({ sqliteDirectory, migrationsFolder });
    try {
