...
describe('legacy Session history backfill', () => {
  it('rebuilds semantic Assistant and Tool Result messages in call order', () => {
    ...
    const migrated = openAndMigrate(fixture.sqliteDirectory, migrationsRoot);
    try {
      ...
      const chain = migrated.prepare<{ message_id: string }>({ sql: `
      ...
      expect(tableExists(migrated, 'agent_run_runtime_events')).toBe(false);
      expect(tableExists(migrated, 'agent_runs')).toBe(false);
      expect(tableExists(migrated, 'artifacts')).toBe(false);
      expect(tableExists(migrated, 'memory_records')).toBe(false);
      expect(migrated.prepare({ sql: 'PRAGMA foreign_key_check' }).all()).toEqual([]);
    } finally {
      migrated.close();
    }
  });
  ...
  it('rolls back staging and preserves legacy tables when a payload is malformed', () => {
    ...
    expect(() => openAndMigrate(fixture.sqliteDirectory, migrationsRoot)).toThrow(DatabaseMigrationError);

    const database = createDatabase({ filename: path.join(fixture.sqliteDirectory, 'megumi.sqlite3') });
    try {
      ...
      expect(database.prepare<{ count: number }>({
        sql: 'SELECT COUNT(*) AS count FROM session_messages',
      }).get()!.count)
        .toBe(2);
  ...
  it('attaches a cancelled partial Assistant message as the active leaf when no final message exists', () => {
    const fixture = createLegacyFixture();
    seedLegacyRun(fixture.database);
    runStatements(fixture.database, [
    ...
    const migrated = openAndMigrate(fixture.sqliteDirectory, migrationsRoot);
    try {
      const active = migrated.prepare<{ message_kind: string; message_json: string }>({ sql: `
  ...
  it('fills a partially migrated run and remains idempotent on the next startup', () => {
    ...
    const first = openAndMigrate(fixture.sqliteDirectory, migrationsRoot);
    expect(first.prepare<{ count: number }>({ sql: 'SELECT COUNT(*) AS count FROM session_messages' }).get()!.count)
      .toBe(5);
    first.close();

    const second = openAndMigrate(fixture.sqliteDirectory, migrationsRoot);
    try {
      expect(second.prepare<{ count: number }>({ sql: 'SELECT COUNT(*) AS count FROM session_messages' }).get()!.count)
        .toBe(5);
    } finally {
      second.close();
    }
  ...
  it('retains an orphaned Tool Result as a semantic fact before the final Assistant message', () => {
    const fixture = createLegacyFixture();
    seedLegacyRun(fixture.database);
    fixture.database.prepare({ sql: "DELETE FROM agent_run_runtime_events WHERE event_id = 'event:call2'" }).run();
    fixture.database.close();

    const migrated = openAndMigrate(fixture.sqliteDirectory, migrationsRoot);
    try {
      const activePath = migrated.prepare<{ message_json: string }>({ sql: `
        WITH RECURSIVE active_path(entry_id, parent_entry_id, message_id, depth) AS (
          SELECT entry_id, parent_entry_id, message_id, 0 FROM session_entries
          WHERE entry_id = (SELECT active_entry_id FROM sessions WHERE session_id = 'session:1')
          UNION ALL
        ...
        ORDER BY path.depth DESC
      ` }).all();
      const conversations = activePath.map((row) => JSON.parse(row.message_json) as { tool_call_id?: string; outcome_status?: string });
...
function createLegacyFixture() {
  ...
  return {
    sqliteDirectory,
    database: openAndMigrate(sqliteDirectory, legacyMigrations),
  };
}

function seedLegacyRun(database: DatabaseConnection, malformed = false): void {
  const now = '2026-07-12T00:00:00.000Z';
  runStatements(database, [`
...
function openAndMigrate(sqliteDirectory: string, migrationsFolder: string): DatabaseConnection {
...
function runStatements(database: DatabaseConnection, statements: readonly string[]): void {
