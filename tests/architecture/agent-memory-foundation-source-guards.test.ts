...
describe('agent memory foundation boundaries', () => {
  ...
  it('keeps core memory helpers free from Host and persistence dependencies', () => {
    ...
    expect(offenders(files, /agent-memory\.service|MemoryRepository|@megumi\/db|from ['"]electron['"]|from ['"]node:fs['"]|from ['"]fs['"]|apps\//)).toEqual([]);
  });
  ...
  it('does not implement out-of-scope memory capabilities in 08 foundation', () => {
    ...
    expect(offenders(files, /\bembedding\b|\bvector\b|\brerank\b|knowledge graph|external memory server|subagent memory|team memory|MCP memory|reflection|evaluator|metrics/iu)).toEqual([]);
  });
