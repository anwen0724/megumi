...
describe('coding agent run mainline guards', () => {
  ...
  it('keeps permission snapshot creation out of the agent-run mainline', () => {
    const serviceSource = read('packages/coding-agent/agent-loop/services/agent-run-service.ts');
    const permissionsIndex = read('packages/coding-agent/permissions/index.ts');

    expect(permissionsIndex).toContain('./services/permission-service');
    expect(serviceSource).not.toContain(['createRun', 'PermissionSnapshot'].join(''));
    expect(serviceSource).not.toContain(['toModel', 'PermissionSnapshot'].join(''));
    ...
    expect(serviceSource).not.toContain(['function toModelVisible', 'PermissionSnapshot'].join(''));
