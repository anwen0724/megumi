...
describe('09 session run integration final source guards', () => {
  ...
  it('removes migration-only bridge files for old session run names', () => {
    expect(existing([
      ...
      'packages/db/repos/agent-tool-call.repo.ts',
      'packages/db/repos/agent-agent-loop.repo.ts',
  ...
  it('routes session-run model input construction through ModelCallInputBuildService', () => {
    const sessionRun = source('packages/coding-agent/agent-loop/services/agent-run-service.ts');
