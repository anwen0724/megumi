...
const concreteToolBoundaryTargets = [
  ...
  join(process.cwd(), 'apps/desktop/src/main/services/agent-tool.service.ts'),
  join(process.cwd(), 'apps/desktop/src/main/ipc/handlers/agent-tool.handler.ts'),
...
describe('agent tool approval foundation source guards', () => {
  it('keeps model-facing tool names Claude-compatible and avoids dotted examples in production code', () => {
    const offenders = collectProductionFiles().filter((file) => {
      ...
      return source.includes('workspace.file.read') || source.includes('agent.plan.byRun.get');
    });
