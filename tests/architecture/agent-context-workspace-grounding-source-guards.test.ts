...
describe('agent context workspace grounding source guards', () => {
  it('keeps shared context contracts platform-independent', () => {
    const offenders = filesUnder('packages/shared')
      .filter((file) => projectPath(file).includes('agent-context-contracts'))
      .filter((file) => {
  ...
  it('keeps DB context repository from doing relevance decisions or workspace reads', () => {
    const offenders = filesUnder('packages/db')
      .filter((file) => projectPath(file).includes('agent-context'))
      .filter((file) => {
        const source = readProjectFile(file);
