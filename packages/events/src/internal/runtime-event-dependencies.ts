/*
 * Serialization-only dependency contracts used by event payload families.
 * They intentionally do not import business Owner packages.
 */
import { z } from 'zod';
import { JsonObjectSchema, type JsonObject } from '@megumi/ai';
...
export const ModelInputContextSourceRefSchema = z.object({
...
export const RUN_STATUSES = ['running', 'waiting', 'cancelling', 'completed', 'failed', 'cancelled'] as const;
...
export interface ContextPatchRequestedPayload {
  patchId: string;
  operation: (typeof CONTEXT_PATCH_OPERATIONS)[number];
  requestedBy: (typeof CONTEXT_PATCH_REQUESTERS)[number];
  reason: string;
}
...
export type PermissionDecision = JsonObject;
export type ToolPolicyDecision = JsonObject;
export type ToolExecution = JsonObject;
