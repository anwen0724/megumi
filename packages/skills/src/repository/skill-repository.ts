...
export type SkillDatabase = DatabaseConnection;
...
export class SkillRepository {
  ...
  findAvailability(input: { skillPath: string }): SkillAvailability | undefined {
    const row = this.database.prepare<SkillAvailabilityRow>({ sql: `
  ...
  listAvailability(): SkillAvailability[] {
    const rows = this.database.prepare<SkillAvailabilityRow>({ sql: `
