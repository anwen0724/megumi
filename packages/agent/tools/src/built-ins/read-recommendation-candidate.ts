/* Defines Recommendation's local, read-only Candidate content Tool. */
...
export interface ReadRecommendationCandidateOperation {
  readRecommendationCandidate(request: {
...
export const readRecommendationCandidateToolDefinition = {
...
export function createReadRecommendationCandidateToolHandler(
  operation: ReadRecommendationCandidateOperation,
): ToolHandler<BuiltInToolContext> {
  return {
    toolName: 'read_recommendation_candidate',
    operations: () => [],
    execute: (_context, invocation, options = {}) => operation.readRecommendationCandidate({
