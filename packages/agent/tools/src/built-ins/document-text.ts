...
import type { WorkspaceFileAccess } from './workspace-file-access';
...
export async function extractFileText(
  ...
  if (extension === '.pdf') return extractPdfText(fileAccess, targetPath, signal);
  if (extension === '.docx') return extractDocxText(fileAccess, targetPath, signal);
  if (extension === '.txt' || extension === '.md' || extension === '.markdown' || !extension) {
    return fileAccess.readFile({ path: targetPath, signal });
  }
  return fileAccess.readFile({ path: targetPath, signal });
}
...
async function extractDocxText(
  ...
  const source = await fileAccess.readBinaryFile({ path: targetPath, signal });
