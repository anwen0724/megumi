...
export const discoveryRecommendationStates = sqliteTable('discovery_recommendation_states', {
  ...
  uniqueIndex('idx_recommendation_states_reaction_sequence').on(table.reactionSequence).where(sql`${table.reactionSequence} > 0`),
...
export const discoveryPreferenceSets = sqliteTable('discovery_preference_sets', {
  ...
  check('check_preference_sets_processed', sql`${table.processedRevision} IS NULL OR (${table.processedRevision} >= 0 AND ${table.processedRevision} <= ${table.revision})`),
  check('check_preference_sets_policy', sql`${table.policyRevision} >= 0`),
  check('check_preference_sets_outcome', sql`${table.lastOutcome} IN ('changed','unchanged','insufficient')`),
...
export const discoveryPreferences = sqliteTable('discovery_preferences', {
  ...
  check('check_preferences_status', sql`${table.status} IN ('active','needs_review','retired','deleted')`),
  ...
  check('check_preferences_ownership', sql`(${table.origin}='learned' AND ${table.polarity} IS NOT NULL AND ${table.dimension} IS NOT NULL AND ${table.userEditedAt} IS NULL) OR (${table.origin}='user' AND ${table.polarity} IS NULL AND ${table.dimension} IS NULL AND ${table.userEditedAt} IS NOT NULL AND ${table.status} IN ('active','deleted'))`),
  check('check_preferences_deletion', sql`(${table.status}='deleted' AND ${table.deletedAt} IS NOT NULL AND ${table.deletedFeedbackSequence} IS NOT NULL) OR (${table.status}<>'deleted' AND ${table.deletedAt} IS NULL AND ${table.deletedFeedbackSequence} IS NULL)`),
  index('idx_preferences_set').on(table.preferenceSetId, table.status, table.origin),
  index('idx_preferences_deleted_feedback').on(table.preferenceSetId, table.deletedFeedbackSequence),
]);
export const discoveryPreferenceEvidence = sqliteTable('discovery_preference_evidence', {
  ...
  recommendationId: text('recommendation_id').notNull().references(() => discoveryRecommendations.id, { onDelete: 'restrict' }),
  ...
  check('check_preference_evidence_explanation', sql`${table.explanation} IS NULL OR length(trim(${table.explanation})) BETWEEN 1 AND 1000`),
  check('check_preference_evidence_quote', sql`${table.contentQuote} IS NULL OR length(${table.contentQuote}) BETWEEN 1 AND 2000`),
  uniqueIndex('idx_preference_evidence_pair').on(table.preferenceId, table.recommendationId),
  index('idx_preference_evidence_recommendation').on(table.recommendationId, table.reactionRevision),
]);
