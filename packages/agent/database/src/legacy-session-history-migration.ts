...
export function prepareLegacySessionHistoryMigration(database: DatabaseConnection): void {
  if (!tableExists(database, 'agent_run_runtime_events')) return;
  database.transaction({
    operation: () => {
      createStagingTable(database);
      database.prepare({ sql: 'DELETE FROM legacy_session_message_staging' }).run();
...
function projectRun(events: readonly LegacyEventRow[], existingFinalText?: string): StagedProjection[] {
...
function createStagingTable(database: DatabaseConnection): void {
  database.prepare({ sql: `
...
function tableExists(database: DatabaseConnection, table: string): boolean {
  return Boolean(database.prepare({ sql: `
