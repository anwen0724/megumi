...
export interface DiscoveryRepository {
  migrateRecommendationIdentities(): { readonly migrated: number; readonly duplicates: number };
  ...
  return {
    migrateRecommendationIdentities() {
      return options.database.transaction({
        operation: () => migrateRecommendationIdentities(options.database),
      });
    },
...
interface RecommendationIdentityRow extends DatabaseRow {
  recommendation_id: string;
  content_identity: string;
  canonical_url: string;
}

function migrateRecommendationIdentities(
  database: DatabaseConnection,
): { readonly migrated: number; readonly duplicates: number } {
