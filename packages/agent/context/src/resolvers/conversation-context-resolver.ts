/* Resolves Session-backed conversation facts without changing the established conversation semantics. */
import type { Api, Model } from '@megumi/ai';
import type {
  EffectiveInstructions,
  InstructionReader,
  SystemInstructionDocument,
} from '@megumi/instructions';
...
export interface ResolveConversationContextRequest {
...
export interface ConversationResolvedContext {
  readonly kind: 'conversation';
...
export type ResolveConversationContextResult =
  | { readonly status: 'resolved'; readonly context: ConversationResolvedContext }
  | { readonly status: 'failed'; readonly failure: ContextFailure };

export interface ConversationContextResolver {
  resolve(request: ResolveConversationContextRequest): Promise<ResolveConversationContextResult>;
}

export interface ConversationContextResolverDependencies {
...
export function createConversationContextResolver(
  dependencies: ConversationContextResolverDependencies,
...
> {
  try {
    return {
      status: 'ok',
      instructions: await dependencies.instructionReader.getSystemInstructions('conversation'),
    };
  } catch (error) {
    return buildFailedContextResult(buildSourceContextFailure({
...
function cancelledResult(): ResolveConversationContextResult {
