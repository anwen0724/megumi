...
export class WorkspaceRepositoryMethods {
  constructor(private readonly database: MegumiDatabase) {}

  listProjects(): ProjectRecord[] {
    return (this.database
      .prepare('SELECT * FROM workspaces ORDER BY last_opened_at DESC, name ASC')
  ...
  getProjectByRepoPathKey(repoPathKey: string): ProjectRecord | undefined {
    const row = this.database
      .prepare('SELECT * FROM workspaces WHERE root_path_key = ?')
  ...
  upsertFromRepoPath(input: ProjectUpsertInput): ProjectRecord {
    ...
    const row: ProjectRow = {
      workspace_id: existing?.projectId ?? createProjectIdFromRepoPathKey(repoPathKey),
      name: basenameProjectRepoPath(repoPath, platform),
      root_path: repoPath,
      root_path_key: repoPathKey,
    ...
    const saved = this.getProject(row.workspace_id);
  ...
  touchProject(projectId: string, openedAt: string): ProjectRecord | undefined {
    this.database
      .prepare('UPDATE workspaces SET updated_at = @opened_at, last_opened_at = @opened_at WHERE workspace_id = @project_id')
      .run({
  ...
  updateStatus(projectId: string, status: ProjectStatus): ProjectRecord | undefined {
    this.database
      .prepare('UPDATE workspaces SET status = @status WHERE workspace_id = @project_id')
      .run({
        project_id: projectId,
        status,
      });
...
export class WorkspaceRepository extends WorkspaceRepositoryMethods {
  upsertFromRootPath(input: WorkspaceUpsertInput): ProjectRecord {
    return this.upsertFromRepoPath({
...
function fromProjectRow(row: ProjectRow): ProjectRecord {
  return {
    ...
    name: row.name,
    repoPath: row.root_path,
    repoPathKey: row.root_path_key,
    status: row.status,
