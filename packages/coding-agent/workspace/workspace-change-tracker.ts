...
import {
  ...
  inputRecord,
  normalizeWorkspaceSlash,
} from './path-policy';

export interface WorkspaceChangeTrackerFileSystem {
...
export interface WorkspaceChangeTrackerOptions {
  projectRoot: string;
  fileSystem: WorkspaceChangeTrackerFileSystem;
...
function normalizeProjectPath(relativePath: string): string {
  const normalized = normalizeWorkspaceSlash(relativePath);
