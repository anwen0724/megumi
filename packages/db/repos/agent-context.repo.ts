...
export class AgentContextRepository {
  ...
  saveBaseline(context: AgentContext): AgentContext {
    this.database.prepare(`
      INSERT INTO agent_context_baselines (
  ...
  getBaseline(contextId: string): AgentContext | undefined {
    const row = this.database
      .prepare('SELECT context_json FROM agent_context_baselines WHERE context_id = ?')
      .get(contextId) as BaselineRow | undefined;
    return row ? parseJson<AgentContext>(row.context_json) : undefined;
  }

  saveSourceRef(source: ContextSourceRef & { runId?: string }): ContextSourceRef {
    ...
    this.database.prepare(`
      INSERT INTO context_source_refs (
  ...
  listSourcesByRun(runId: string): ContextSourceRef[] {
    return (this.database
      .prepare('SELECT metadata_json AS source_json FROM context_source_refs WHERE run_id = ? ORDER BY loaded_at ASC')
      .all(runId) as SourceRow[]).map((row) => parseJson<ContextSourceRef>(row.source_json));
  }
  ...
  listPatchesByRun(runId: string): ContextPatch[] {
    return (this.database
      .prepare('SELECT metadata_json AS patch_json FROM context_patches WHERE run_id = ? ORDER BY created_at ASC')
      .all(runId) as PatchRow[]).map((row) => parseJson<ContextPatch>(row.patch_json));
  }

  saveEffectiveBuild(build: EffectiveContextBuild): EffectiveContextBuild {
    assertSafeBuildMetadata(build.metadata);

    this.database.prepare(`
      INSERT INTO effective_context_builds (
  ...
  listEffectiveBuildsByRun(runId: string): EffectiveContextBuild[] {
    return (this.database
      .prepare('SELECT metadata_json AS build_json FROM effective_context_builds WHERE run_id = ? ORDER BY built_at ASC')
      .all(runId) as BuildRow[]).map((row) => parseJson<EffectiveContextBuild>(row.build_json));
  }
}
