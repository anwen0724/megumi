...
/** Provider catalogs intentionally shipped by Megumi. */
const RETAINED_PROVIDER_IDS = new Set<string>([
	"anthropic",
	"deepseek",
	"google",
	"huggingface",
...
type OpenAICompletionsResolvedCompat = Required<
	Omit<OpenAICompletionsCompat, "cacheControlFormat" | "deferredToolsMode">
> & {
	cacheControlFormat?: OpenAICompletionsCompat["cacheControlFormat"];
};

const OPENAI_COMPLETIONS_DEFAULT_COMPAT: OpenAICompletionsResolvedCompat = {
	supportsStore: true,
...
function applyThinkingLevelMetadata(model: Model<any>): void {
	...
	if (model.provider === "openrouter" && model.id.startsWith("inception/mercury-2")) {
		...
		// Megumi's low/medium/high pass through verbatim; OpenRouter normalizes to Mercury's vocabulary.
		mergeThinkingLevelMap(model, { off: null });
	}
	...
	if (model.provider === "github-copilot") {
		const override = (
			GITHUB_COPILOT_THINKING_LEVEL_OVERRIDES as Record<
				string,
...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		const models: Model<any>[] = [];
		// Megumi does not ship the NVIDIA provider, so catalog generation must not
		// perform an unrelated NVIDIA endpoint request.
		const nvidiaNimModelIds = new Map<string, string>();
		...
		// "alibaba-token-plan[-cn]"; Megumi exposes them as "qwen-token-plan[-cn]".
...
async function generateModels() {
	...
	// OpenRouter supplies its own dynamic catalog in addition to models.dev.
	...
	for (const model of allModels) {
		if (!RETAINED_PROVIDER_IDS.has(model.provider)) continue;
...
`;
	...
	// Print statistics
	const retainedModels = Object.values(providers).flatMap((models) => Object.values(models));
	const totalModels = retainedModels.length;
	const reasoningModels = retainedModels.filter((model) => model.reasoning).length;
