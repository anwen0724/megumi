...
export const stream: StreamFunction<"openai-codex-responses", OpenAICodexResponsesOptions> = (
	...
	(async () => {
		...
		try {
			...
			const sseBody: BodyInit = compressedBody ? new Uint8Array(compressedBody) : bodyJson;
...
function buildBaseCodexHeaders(
	...
	headers.set("originator", "megumi");
	const userAgent = _os ? `megumi (${_os.platform()} ${_os.release()}; ${_os.arch()})` : "megumi (browser)";
