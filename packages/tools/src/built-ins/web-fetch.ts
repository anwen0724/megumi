...
export const webFetchToolDefinition: ToolDefinition = {
  name: 'web_fetch',
  description: 'Read an HTTP(S) page and return size-limited text content (up to 9,000 bytes).',
  promptSnippet: 'Read an HTTP(S) page and return text content.',
  parameters: {
    type: 'object',
  ...
  outputSchema: {
    type: 'object',
    properties: {
      requestedUrl: { type: 'string' }, finalUrl: { type: 'string' },
      title: { type: 'string' }, contentType: { type: 'string' },
...
export async function executeWebFetch(
  ...
  if (!context.webFetch) throw new Error('web_fetch is unavailable.');
  const requestedUrl = requireString(inputRecord(input), 'url').trim();
  const result = await context.webFetch.fetch({ url: requestedUrl, signal });
  return {
    outputKind: 'json',
    content: result,
