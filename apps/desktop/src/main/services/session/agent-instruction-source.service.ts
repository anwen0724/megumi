  ...
  private async loadFirstReadableCandidate(
    ...
    for (const fileName of AGENT_INSTRUCTION_SOURCE_CANDIDATES) {
      ...
      try {
        ...
        const actualFileName = await actualDirectoryFileName(input.directoryPath, fileName);
...
async function actualDirectoryFileName(directoryPath: string, candidateName: string): Promise<string> {
  try {
