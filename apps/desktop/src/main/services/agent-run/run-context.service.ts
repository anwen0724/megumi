...
import { composeCodingAgentPersistence } from '@megumi/coding-agent/composition';
import type { RunContextRepository } from '@megumi/coding-agent/persistence/repos/run-context.repo';
import {
  RunContextService,
...
export function createDefaultRunContextService(
  ...
  const contextRepository = options.repository ?? composeCodingAgentPersistence(homePaths).runContextRepository;
