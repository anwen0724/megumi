...
const out=path.join(audit,phase==='pilot'?'attempt-01-pilot':'attempt-02-formal');
...
const model:unknown=JSON.parse(await readFile(path.join(root,'.megumi/audits/preference-model.json'),'utf8'));
const identities=phase==='pilot'?['controlled/resume-recommendation.photo-120']:Object.keys(labels).sort();
const repetitions=phase==='pilot'?[0]:[1,2];
...
for(const repetition of repetitions)for(const identity of identities)for(const arm of (repetition%2===0?['full','layered']:['layered','full'])) {
  ...
  try {
    const run=await runEvaluation({repositoryRoot:root,evaluationRoot:path.join(out,'evaluation'),datasetRoot:path.join(audit,'datasets',arm),request:EvaluationRunRequestSchema.parse({caseIds:[identity],candidateModel:model,safetyWallClockLimitMs:240000})});
    ...
    const summary={key,phase,repetition,arm,identity,domain:gold.domain,poolSize:gold.poolSize,targetCount:gold.targetCount,runDirectory:run.runDirectory,
