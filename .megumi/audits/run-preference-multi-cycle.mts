...
const output = path.join(root, '.megumi/audits/preference-sequence-multi-cycle');
await mkdir(output, { recursive: true });
const outcomes = await Promise.allSettled([1, 2, 3].map(async (repetition) => {
  console.log(`Starting final repetition ${repetition}/3, one two-cycle case.`);
  const result = await runEvaluation({ repositoryRoot: root, evaluationRoot: path.join(root, 'evals/agent'), datasetRoot: path.join(root, 'evals/agent/datasets'),
    request: EvaluationRunRequestSchema.parse({ caseIds: ['controlled/preference-sequence.photo-accumulation'], candidateModel, safetyWallClockLimitMs: 240000 }),
  });
