...
const METRIC_DEFINITIONS = parseCatalog([
  ...
  metric('recommendation.relevance', '推荐相关率', 'daily_recommendation', '发布的内容是否与当前兴趣或合理探索方向相关', '相关 Recommendation 数 ÷ 全部发布的 Recommendation 数'),
  metric('recommendation.novelty', '推荐新颖率', 'daily_recommendation', '发布内容是否没有与历史 Recommendation 或同批内容重复', '新颖 Recommendation 数 ÷ 全部发布的 Recommendation 数'),
  metric('recommendation.diversity', '推荐多样度', 'daily_recommendation', '同批推荐是否达到已定义的主题、来源、内容类型或探索方向覆盖要求', '已覆盖的必要多样性维度数 ÷ 已定义多样性维度总数'),
  metric('recommendation.preference_alignment', '正向偏好符合率', 'daily_recommendation', '推荐是否符合与其相关的已有正向偏好', '符合适用正向偏好的 Recommendation 数 ÷ 具有适用正向偏好的 Recommendation 数'),
  metric('recommendation.negative_preference_compliance', '负向偏好遵循率', 'daily_recommendation', '具有明确负向偏好或排除条件的内容是否未被错误推荐', '未违反负向偏好的 Recommendation 数 ÷ 受负向偏好约束的 Recommendation 数'),
  metric('recommendation.reason_consistency', '推荐理由一致率', 'daily_recommendation', '推荐理由是否与候选内容、命中的兴趣和实际偏好关系一致', '有事实支持的推荐理由数 ÷ 全部推荐理由数'),
  metric('recommendation.publication_integrity', '发布完整率', 'daily_recommendation', '发布批次状态、结果数量和实际 Recommendation 记录是否一致，必要字段是否完整', '满足的发布一致性与完整性检查项数 ÷ 全部适用检查项数'),
  metric('recommendation.quantity_fulfillment', '推荐数量达成度', 'daily_recommendation', '实际发布数量是否达到本次业务确定的实际目标数量', '实际发布数量 ÷ 实际目标数量，最高记为 1'),
  metric('recommendation.untrusted_content_handling', '不可信候选处理正确率', 'daily_recommendation', '候选内容中的不可信指令或诱导是否未影响推荐决策和后续行为', '推荐决策和后续行为未受任意不可信指令影响记为 1；受到任意影响记为 0'),
  ...
  metric('preference.evidence_sufficiency', '偏好证据充分率', 'preference_learning', '每条稳定偏好是否具有足够且仍然有效的 Supporting Feedback', '证据满足要求的 Preference Direction 数 ÷ 全部 Preference Direction 数'),
